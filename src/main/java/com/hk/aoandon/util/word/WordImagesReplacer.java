 ...
 */
@Log4j2
public class WordImagesReplacer implements WordReplacer {
    @Override
    ...
    public boolean replace(String text, Map<String, Object> data, XWPFRun run) {
        ...
        if (!StringUtils.isEmpty(data.get(imgKey))) {
            List<WordImage> imgDatas = (List<WordImage>) data.get(imgKey);

            for (int i = 0; i < imgDatas.size(); i++) {
                Integer pos = (i == 0 ? 0 : null);
                WordImage imgData = imgDatas.get(i);
                try {
                    if (pos == null) {
                        run.setText(StringUtils.isEmpty(imgData.getTitle()) ? "" : "\n"+imgData.getTitle());
                    } else {
                        run.setText(StringUtils.isEmpty(imgData.getTitle()) ? "" : "\n"+imgData.getTitle(), pos);
                    }
