...
public class PoiWordToolsDynamic {
    ...
    public static boolean refreshBarStrGraphContent(CTBarChart barChart,
        ...
        for (int i = 0; i < culomnNum; i++) {
            ...
            for (Map<String, String> stringStringMap : dataList) {
                //判断获取的值是否为空
                String value = "0";
                new BigDecimal(stringStringMap.get(fldNameArr.get(i + position)));
    ...
    public static boolean refreshLineStrGraphContent(CTLineChart ctLineChart,
        ...
        for (int i = 0; i < culomnNum; i++) {
            ...
            for (Map<String, String> stringStringMap : dataList) {
                //判断获取的值是否为空
                String value =  new BigDecimal(stringStringMap.get(fldNameArr.get(i + position))).toString();
                if (!"0".equals(value)) {
                    CTNumVal numVal = numData.addNewPt();
                    numVal.setIdx(idx);
                    numVal.setV(value);
