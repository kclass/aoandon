...
import com.hk.aoandon.util.word.BaseWordTableReplacer;
import com.hk.aoandon.util.word.WordTableStyle;
 ...
 */
public class GapWordTableReplacer extends BaseWordTableReplacer {
    public GapWordTableReplacer(WordTableStyle wordTableStyle) {
        super(wordTableStyle);
    }

    private static final List<String> REPLACE_SYMBOL = Arrays.asList("areaCoverStatistic", "areaFluxStatistic", "areaUserStatistic");

    @Override
    protected boolean createTable(XWPFDocument doc, XWPFParagraph paragraph, Object data, String key) throws Exception {
        if (REPLACE_SYMBOL.contains(key)) {
            ...
            for (int i = 1; i < tableData.size(); i++) {
                ...
                for (int j = 0; j < keys.size(); j++) {
                    ...
                    if ((i % 2) == 0) {
                        style.setHeadStyle(tableOneRowTwo.getCell(j), content == null ? "" : content.toString());
                    } else {
